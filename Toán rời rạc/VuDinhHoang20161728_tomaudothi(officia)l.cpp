...
void coloring(); //su dung thuat giai Welsh Powell
...
void output()
{
	...
	int interconnect = 0;
	int count = 0;
	char *Mark = new char[V];
	char success ;
	for(int i = 0; i < V; i++)
	Mark[i] = 1;		//danh dau dinh dau
	count++;	//dem so dinh da danh dau
	do
	{
		success = 1; //khi khong con kha nang nao
		for(int i = 0; i < V; i++)
			if(Mark[i] == 1)
			{
				for(int j = 0; j < V; j++)
					if(Mark[j] == 0 && a[i][j] > 0){
						...
						success = 0;
						count++;
						if(count == V) cout<<"Do thi lien thong"<<endl;
					}
			}
	}while(success = 0);
	if(count != V) cout<<"Do thi khong lien thong"<<endl;
}
...
void verLevel()
{
	...
	for(int i = 1; i < V; i++)
	{
		if(vLevel[i] != vLevel[i+1])
		{
			cout<<"Do thi khong chinh quy"<<endl;
			mark = 0;
			break;
		}
	}
	if(mark) cout<<"Do thi chinh quy"<<endl;
	//tim bac lon nhat
	for(int i = 1; i <= V; i++)
	{
		if(vLevel[i] > MaxLevel){
			MaxLevel = vLevel[i]; 
		}
	}
	cout<<"Bac cac dinh cua do thi nho hon bang k = "<<MaxLevel<<endl;
}
